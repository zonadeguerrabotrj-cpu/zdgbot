...
// =====================
// API POST com timeout e tratamento de erro
// =====================
async function apiPost(endpoint, body) {
  try {
    ...
    console.error("❌ Erro na apiPost:", err);
    return null;
  }
}
...
async function apiGetPublic(endpointWithQuery) {
  try {
    ...
    console.error("❌ Erro na apiGetPublic:", err);
...
function genCodeString(){ const chars="ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"; let out=""; for(let i=0;i<10;i++) out+=chars[Math.floor(Math.random()*chars.length)]; return out; }
function normalizeTypeInput(type){ return String(type||"").trim(); }
function normalizeValueInput(value){ if(typeof value==="boolean") return value; const v=String(value??"").trim(); if(v.toLowerCase()==="true") return true; if(v.toLowerCase()==="false") return false; if(/^\d+$/.test(v)) return Number(v); return v; }

// =====================
// SLASH COMMANDS
// =====================
const commands = [
  new SlashCommandBuilder().setName("gencode").setDescription("Gera códigos").addStringOption(o=>o.setName("type").setDescription("Ex: IMI Galil | money | vip").setRequired(true)).addStringOption(o=>o.setName("value").setDescription("Valor ou true/false").setRequired(true)).addIntegerOption(o=>o.setName("amount").setDescription("Quantidade máx 100").setRequired(true)),
  new SlashCommandBuilder().setName("genrand").setDescription("Gera códigos aleatórios").addIntegerOption(o=>o.setName("amount").setDescription("Quantidade máx 100").setRequired(true)),
  new SlashCommandBuilder().setName("ban").setDescription("Banir jogador").addStringOption(o=>o.setName("identificador").setDescription("UserId ou @username").setRequired(true)).addStringOption(o=>o.setName("duração").setDescription("perma | 30m | 2h | 7d").setRequired(true)).addStringOption(o=>o.setName("motivo").setDescription("Motivo do ban").setRequired(true)),
  new SlashCommandBuilder().setName("unban").setDescription("Remover ban").addStringOption(o=>o.setName("identificador").setDescription("UserId ou @username").setRequired(true)).addStringOption(o=>o.setName("motivo").setDescription("Motivo do unban").setRequired(true)),
  new SlashCommandBuilder().setName("kick").setDescription("Kick").addStringOption(o=>o.setName("identificador").setDescription("UserId ou @username").setRequired(true)).addStringOption(o=>o.setName("motivo").setDescription("Motivo do kick").setRequired(true))
].map(c=>c.toJSON());
...
async function sendRedeemLogIfNew() {
  ...
  const emb = new EmbedBuilder()
    ...
    ).setFooter({text:"Code Redeem System"}).setTimestamp(new Date(Number(json.createdAt||Date.now())));
  ch.send({embeds:[emb]});
}
...
client.on("interactionCreate",async interaction=>{
  ...
  try {
    if(interaction.commandName==="gencode"){
      ...
      if(amount<1||amount>100) return interaction.reply({embeds:[denyEmbed("Quantidade inválida","Amount deve ser entre 1 e 100.")], ephemeral:true});
      ...
      const emb = infoEmbed(`${amount} Código(s) gerado(s)`).addFields(
    ...
    if(interaction.commandName==="genrand"){
      ...
      const emb = infoEmbed(`${amount} Código(s) aleatório(s)`).addFields(
    ...
    // ======== ban / unban / kick ========
    if(["ban","unban","kick"].includes(interaction.commandName) && !isMod){
      return interaction.reply({embeds:[denyEmbed("Sem permissão","Você não tem permissão pra usar moderação.")], ephemeral:true});
    }
    ...
    if(interaction.commandName==="ban"){
      ...
      const emb = new EmbedBuilder().setTitle("Ban aplicado").setColor(0xED4245).addFields(
    ...
    if(interaction.commandName==="unban"||interaction.commandName==="kick"){
      const identificador = interaction.options.getString("identificador",true);
      const motivo = interaction.options.getString("motivo",true);
      await apiPost(interaction.commandName,{robloxUser:identificador,reason:motivo,adminDiscord:`${interaction.user.username} (${interaction.user.id})`});
      const emb = new EmbedBuilder().setTitle(`${interaction.commandName.charAt(0).toUpperCase()+interaction.commandName.slice(1)} aplicado`).setColor(interaction.commandName==="kick"?0xFEE75C:0x57F287).addFields(
        {name:"Identificador",value:`\`${identificador}\``,inline:false},
        {name:"Motivo",value:motivo,inline:false},
        {name:"Moderator",value:`${interaction.user.username} (${interaction.user.id})`,inline:false}
      ).setTimestamp(new Date());
    ...
    if(interaction.deferred || interaction.replied){
      return interaction.editReply({embeds:[denyEmbed("Erro interno",String(err))]}).catch(()=>{});
    }
    return interaction.reply({embeds:[denyEmbed("Erro interno",String(err))], ephemeral:true}).catch(()=>{});
  }
});
